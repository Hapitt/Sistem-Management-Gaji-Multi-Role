<x-layouts.app :title="'Edit Karyawan - ' . $karyawan->nama">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Perbarui informasi karyawan {{ $karyawan->nama }} sesuai kebutuhan perusahaan.
                </p>
            </div>
        ...
        <div class="p-6">
            <form action="{{ route('admin.karyawan.update', $karyawan->id_karyawan) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="md:col-span-2">
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('nama') border-red-500 @enderror" 
                               id="nama" name="nama" value="{{ old('nama', $karyawan->nama) }}" placeholder="Masukkan nama lengkap" required>
                        @error('nama')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="md:col-span-2">
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('divisi') border-red-500 @enderror" 
                               id="divisi" name="divisi" value="{{ old('divisi', $karyawan->divisi) }}" placeholder="Masukkan divisi" required>
                        @error('divisi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="md:col-span-2">
                        <textarea class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('alamat') border-red-500 @enderror" 
                                  id="alamat" name="alamat" rows="3" placeholder="Masukkan alamat lengkap" required>{{ old('alamat', $karyawan->alamat) }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="md:col-span-2">
                        <input type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('umur') border-red-500 @enderror" 
                               id="umur" name="umur" value="{{ old('umur', $karyawan->umur) }}" placeholder="Masukkan umur" min="18" max="65" required>
                        @error('umur')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            <div class="mt-3 flex items-center gap-4">
                                <div class="relative">
                                    <img src="{{ asset('storage/' . $karyawan->foto) }}" 
                                         alt="Foto Saat Ini" 
                                         class="w-20 h-20 rounded-lg border border-gray-300 object-cover">
                                </div>
                ...
                <div class="flex justify-end pt-6 border-t border-gray-200">
                    <a href="{{ route('admin.karyawan.index') }}" class="inline-flex items-center px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium rounded-lg transition-colors mr-4">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                        </svg>
                        Kembali
